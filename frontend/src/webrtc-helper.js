...
class WebRTCPeer {
  constructor(options = {}) {
    ...
    console.log(`[WebRTC] Creating peer connection (initiator: ${this.initiator})`);
    ...
    try {
      ...
      if (this.stream && this.stream.getTracks) {
        console.log(`[WebRTC] Adding ${this.stream.getTracks().length} local tracks to connection`);
        this.stream.getTracks().forEach((track) => {
          this.pc.addTrack(track, this.stream);
        });
      } else if (this.stream) {
        console.log("[WebRTC] Stream provided but getTracks method not available");
      } else {
  ...
  async _createOffer(options = {}) {
    try {
      console.log("[WebRTC] Creating offer", options.iceRestart ? "(ICE restart)" : "");
    ...
    try {
      ...
      if (data.type === "sdp") {
        ...
        try {
          ...
          console.log(`[WebRTC] Remote description set successfully (${sdpType})`);
      ...
      // Handle ICE candidate
        ...
        if (this.pc.remoteDescription && this.pc.remoteDescription.type) {
          ...
          console.log("[WebRTC] Queueing ICE candidate (remote description not yet set)");
  ...
  async _processQueuedCandidates() {
    ...
    if (!this.pc || !this.pc.remoteDescription) {
      console.warn("[WebRTC] Cannot process queued candidates: no remote description set");
      return;
    }
    
    console.log(`[WebRTC] Processing ${this._queuedCandidates.length} queued ICE candidates`);
    ...
    for (const candidate of candidates) {
      try {
        ...
        if (!this.pc.remoteDescription) {
          console.log("[WebRTC] Remote description lost, re-queueing candidate");
